...
@ReactConfirmDialog = React.createClass
  ...
  statics:
    ...
    # Example: ReactConfirmDialog.ask('Are you sure?', {description: 'really?' }).then(function(){alert('OK')}).fail(function(){alert('CANCEL')})
    ask: (message, options = {}) ->
      props = $.extend({message: message}, options)
      wrapper = document.body.appendChild(document.createElement('div'))
      component = ReactDOM.render(React.createElement(ReactConfirmDialog, props), wrapper)
      cleanup = ->
